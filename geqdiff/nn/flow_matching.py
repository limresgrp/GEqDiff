...
class ForwardFlowMatchingModule(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        # Direction/magnitude coupling has been removed in favor of direct vector prediction.
        # Keep constructor compatibility but disallow activating the legacy behavior.
        if directional_velocity_couplings not in (None, [], ()):
            raise ValueError(
                "directional_velocity_couplings is no longer supported; "
                "use direct vector corrupt/readout fields only."
            )
        _ = directional_velocity_eps
