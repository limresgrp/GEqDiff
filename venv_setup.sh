...
DEFAULT_PYTHON="3.10"
DEFAULT_VENV_DIR="${ROOT_DIR}/.venv-geqdiff"
...
Create a uv-based virtual environment for GEqDiff and install:
- GEqTrain (https://github.com/limresgrp/GEqTrain.git)
- GEqDiff  (this repository, editable)
- extra runtime packages used by the repo scripts and LEGO tools
...
Options:
  ...
  --deps-dir PATH          Directory where GEqTrain is cloned (default: ${DEFAULT_DEPS_DIR})
  ...
  --update-repos           Fetch and update GEqTrain if already cloned
...
install_rdkit() {
  ...
  if uv pip install --python "${python_bin}" rdkit; then
    return
  fi
...
echo "Installing GEqDiff runtime dependencies..."
RUNTIME_DEPS=(
  numpy
  scipy
  tqdm
