...
sample_lego_blocks() {
  ...
  skip_fields_raw="$(prompt_with_default "Skip diffusion fields (comma-separated: pos,shape,dipole; blank=none)" "")"
  ...
  if [[ -n "${skip_fields_raw}" ]]; then
    cmd+=(--skip-diffusion-fields "${skip_fields_raw}")
  fi
  if [[ "${save_intermediates}" == "y" ]]; then
